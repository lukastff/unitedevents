...
@section("content")
    <section class="mx-auto max-w-2xl px-4 py-16 sm:px-6 sm:py-24 lg:px-8">
        ...
        <form action="/events/update/{{ $event->id }}" method="POST" enctype="multipart/form-data" class="mt-12 space-y-8">
            @csrf
            @method("PUT")
            ...
            <div>
                ...
                <textarea id="description" name="description" rows="5" placeholder="O que vai acontecer no evento"
                          class="w-full rounded-xl border border-zinc-200 bg-zinc-50 px-4 py-3 text-sm text-zinc-900 shadow-sm transition-all duration-300 ease-out placeholder:text-zinc-400 focus:border-accent focus:bg-white focus:outline-none focus:ring-2 focus:ring-accent/30">{{ $event->description }}</textarea>
            </div>
